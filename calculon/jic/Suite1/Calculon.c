...
void CopyToTemp(int numProg, char *tempDir) {
   ...
   Test *test;
   char *sourceDirectory = calloc(MAX_CHARS, sizeof(char));
...
Report *RunExec(Test *test, char **argv, char *tempDir) {
   ...
   char *currDirectory = calloc(MAX_CHARS, sizeof(char));

        getcwd(currDirectory, MAX_CHARS);
         printf("runexec was in path = %s\n", currDirectory);
   chdir(tempDir);
   getcwd(currDirectory, MAX_CHARS);
   umask(0);
   printf("runexec is now in path = %s\n", currDirectory);
   printf("the input file I am opening is called |%s|\n", test->inputFile);
   inputFile = open(test->inputFile, O_RDONLY);
      printf("input errno = %d\n", errno);

   outputFile = open("./test.output.temp", O_WRONLY | O_CREAT | O_TRUNC, PERMISSIONS);
   printf("output errno = %d\n", errno);
   errorFile = open("./test.output.temp", O_WRONLY | O_CREAT | O_TRUNC, PERMISSIONS);
      printf("error errno = %d\n", errno);

   printf("values of opening = %d, %d, %d\n", inputFile, outputFile, errorFile);
   //printf("!!!HEY!!! errno returned %d\n", errno);
...
int RunTest(int numProgram, int testNumber, char *tempDir) {
   ...
   char *tFlag = calloc(MAX_CHARS, sizeof(char)), *scaledTFlag = calloc(MAX_CHARS, sizeof(char)), *execName = calloc(MAX_CHARS, sizeof(char)), **argv = calloc(MAX_CHARS, sizeof(char *));
   char *currDirectory = calloc(MAX_CHARS, sizeof(char));
   ...
   for (index = 0; index < test->argc; index++) {
      ...
      printf("runtest is in path = |%s| when it calls run exec\n", currDirectory);

   argv[argc] = NULL;
   report = RunExec(test, argv, tempDir);
             getcwd(currDirectory, MAX_CHARS);

   printf("runtest is in path = |%s| when it finishes run exec\n", currDirectory);
...
void RunProgram(int numProgram) {
   ...
   getcwd(sourceDirectory, MAX_CHARS);
   printf("the source dir is %s and the temp dir is %s\n", sourceDirectory, tempDirectory);
   ...
   getcwd(sourceDirectory, MAX_CHARS);
   printf("about to delete directory named |%s| in |%s|\n", tempDirectory, sourceDirectory);
